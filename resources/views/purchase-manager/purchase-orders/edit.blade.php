        ...
        <div class="col-12">
            <div class="bg-gradient-to-r from-blue-600 to-blue-800 rounded-lg p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-3xl font-bold mb-2">Edit Purchase Order #{{ $purchaseOrder->id }}</h1>
                        <p class="text-blue-100 text-lg">Update purchase order information</p>
                    </div>
                    <div class="flex space-x-3">
                        <a href="{{ route('purchase-manager.purchase-orders.show', $purchaseOrder) }}" class="bg-white text-blue-600 px-6 py-3 rounded-lg font-semibold hover:bg-blue-50 transition-colors">
    ...
    <div class="row">
        <div class="col-12">
            <div class="bg-white rounded-lg shadow-md p-6">
                <form action="{{ route('purchase-manager.purchase-orders.update', $purchaseOrder) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                        ...
                        <div>
                            ...
                            <input type="date" name="order_date" id="order_date" value="{{ old('order_date', $purchaseOrder->order_date ? $purchaseOrder->order_date->format('Y-m-d') : '') }}" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            @error('order_date')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="date" name="expected_delivery_date" id="expected_delivery_date" value="{{ old('expected_delivery_date', $purchaseOrder->expected_delivery_date ? $purchaseOrder->expected_delivery_date->format('Y-m-d') : '') }}" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        ...
                        <div>
                            ...
                            <textarea name="notes" id="notes" rows="3" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Additional notes...">{{ old('notes', $purchaseOrder->notes) }}</textarea>
                            @error('notes')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div id="items-container">
                            ...
                            <div class="item-row grid grid-cols-1 md:grid-cols-5 gap-4 mb-4 p-4 border border-gray-200 rounded-lg">
                                <div>
                                    ...
                                    <select name="items[{{ $index }}][product_id]" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                ...
                                <div>
                                    ...
                                    <input type="number" name="items[{{ $index }}][quantity]" min="1" step="1" value="{{ $item->quantity }}" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="1">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Unit Price *</label>
                                    <input type="number" name="items[{{ $index }}][unit_price]" min="0" step="0.01" value="{{ $item->unit_price }}" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="0.00">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Total Price</label>
                                    <input type="number" name="items[{{ $index }}][total_price]" readonly value="{{ $item->total_price }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-gray-50" placeholder="0.00">
                                </div>
                        ...
                        <div class="flex justify-end">
                            <div class="text-right">
                                <div class="text-2xl font-bold text-gray-900">
                                    Total: ₹<span id="grand-total">{{ number_format($purchaseOrder->items->sum('total_price'), 2) }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
...
@php
    $itemCount = (int) count($purchaseOrder->items ?? []);
@endphp
<script>
document.addEventListener('DOMContentLoaded', function() {
    let itemIndex = parseInt('{{ $itemCount }}') || 0;
