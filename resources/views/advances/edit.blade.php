...
<div class="p-6">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Advance</h1>
            <p class="text-gray-600 dark:text-gray-400">Update advance information</p>
        </div>
        <div class="flex space-x-3">
            <a href="{{ route('advances.show', $advance) }}" class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg transition duration-300">
                View Advance
            </a>
            <a href="{{ route('advances.index') }}" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg transition duration-300">
                Back to Advances
            </a>
        </div>
    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" id="title" name="title" value="{{ old('title', $advance->title) }}" required
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="3"
                                  class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500 dark:bg-gray-700 dark:text-white @error('description') border-red-500 @enderror">{{ old('description', $advance->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select id="advance_type" name="advance_type" required onchange="toggleTypeFields()"
                            ...
                            <option value="employee" {{ old('advance_type', $advance->advance_type) == 'employee' ? 'selected' : '' }}>Employee</option>
                            <option value="vendor" {{ old('advance_type', $advance->advance_type) == 'vendor' ? 'selected' : '' }}>Vendor</option>
                            <option value="project" {{ old('advance_type', $advance->advance_type) == 'project' ? 'selected' : '' }}>Project</option>
                        </select>
                        @error('advance_type')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div id="employee_field" style="display: {{ old('advance_type', $advance->advance_type) == 'employee' ? 'block' : 'none' }};">
                    ...
                    <div id="vendor_field" style="display: {{ old('advance_type', $advance->advance_type) == 'vendor' ? 'block' : 'none' }};">
                        ...
                        <select id="vendor_id" name="vendor_id"
                            ...
                            @foreach($vendors as $vendor)
                                <option value="{{ $vendor->id }}" {{ old('vendor_id', $advance->vendor_id) == $vendor->id ? 'selected' : '' }}>
                                    {{ $vendor->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="project_id" name="project_id"
                            ...
                            @foreach($projects as $project)
                                <option value="{{ $project->id }}" {{ old('project_id', $advance->project_id) == $project->id ? 'selected' : '' }}>
                                    {{ $project->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="number" id="amount" name="amount" value="{{ old('amount', $advance->amount) }}" step="0.01" min="0.01" required
                    ...
                    <div>
                        ...
                        <input type="date" id="advance_date" name="advance_date" value="{{ old('advance_date', $advance->advance_date->format('Y-m-d')) }}" required
                    ...
                    <div>
                        ...
                        <input type="date" id="expected_settlement_date" name="expected_settlement_date" value="{{ old('expected_settlement_date', $advance->expected_settlement_date ? $advance->expected_settlement_date->format('Y-m-d') : '') }}"
                    ...
                    <div>
                        ...
                        <textarea id="purpose" name="purpose" rows="3"
                                  class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500 dark:bg-gray-700 dark:text-white @error('purpose') border-red-500 @enderror">{{ old('purpose', $advance->purpose) }}</textarea>
                        @error('purpose')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="notes" name="notes" rows="3"
                                  class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500 dark:bg-gray-700 dark:text-white @error('notes') border-red-500 @enderror">{{ old('notes', $advance->notes) }}</textarea>
                        @error('notes')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
