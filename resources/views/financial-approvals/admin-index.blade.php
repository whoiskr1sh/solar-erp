...
@section('title', 'Admin Financial Approvals')

@section('content')
<div class="p-6">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Admin Financial Approvals</h1>
            <p class="text-gray-600 dark:text-gray-400">Final review and approval for site expenses and advances</p>
        </div>
    </div>

    <!-- Stats Cards -->
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md border-l-4 border-indigo-500 p-6">
            <div class="flex items-center">
                <div class="flex-1">
                    <p class="text-xs font-bold text-indigo-600 uppercase mb-1">Final Expense Approvals</p>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md border-l-4 border-purple-500 p-6">
            <div class="flex items-center">
                <div class="flex-1">
                    <p class="text-xs font-bold text-purple-600 uppercase mb-1">Final Advance Approvals</p>
    ...
    <div class="mb-6">
        <div class="border-b border-gray-200 dark:border-gray-700">
            <nav class="-mb-px flex space-x-8">
                <a href="{{ route('approvals.admin.index', ['type' => 'site-expenses']) }}" class="{{ $type === 'site-expenses' ? 'border-teal-500 text-teal-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm">
                    Site Expenses
                </a>
                <a href="{{ route('approvals.admin.index', ['type' => 'advances']) }}" class="{{ $type === 'advances' ? 'border-teal-500 text-teal-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm">
                    Advances
                </a>
            </nav>
        </div>
            ...
            <table class="w-full text-sm text-left">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                    <tr>
                        ...
                        <th class="px-6 py-3">HR Approved By</th>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                        ...
                        <td class="px-6 py-4">
                            <div class="text-sm text-gray-900 dark:text-white">{{ $request->hrApprover->name ?? 'N/A' }}</div>
                            <div class="text-xs text-gray-500">{{ $request->hr_approved_at ? $request->hr_approved_at->format('M d, Y') : '' }}</div>
                        </td>
                    ...
                    @empty
                    <tr>
                        <td colspan="7" class="px-6 py-4 text-center text-gray-500">No pending {{ str_replace('-', ' ', $type) }} found for Admin approval.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
